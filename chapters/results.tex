...
\begin{itemize}
    \item Explain why not able to explore. 
    \item How could each model be implemented differently. 
    \item Cellular is slow so should not be happening. 
    \item Pool model; Outside programmers control how threads are run. 
    \item Island; as mentioned. Longer migration interval. 
\end{itemize}


% Major trends
\begin{itemize}
    \item Only Island modal is able to explore, but only for 50 generations. 
    \item Pool efficiecny.
    \item Answer why only optimized settings for master slave.
    \item Answer why not tested different versions. 
    \item Discuss that other models need more time. Island model use more time before it decides on turbines, and therefore gets less time to optimize the solution. Might have reach the level of Pool model?
\end{itemize}


% Other observations
\begin{itemize}
    \item Discuss exploration and ex..
    \item Why are the models bad. Why do they not work better.
    \item Fairness... 
    \item Overfitting parameters.
    \item No enough time to optimize all parameters. 
    \item Island population size - Longer migration rate.
    \item Fitness functin bias. Equal weight.
    \item Discuss time limitations. 
    \item Fairness.
    \item How long it takes to run different models.
    \item Every Island model parameter should be optimized. 
    \item reference that says that Master/Slave model is better might have another version of fairness. Not same number of evaluations and same population size. 
    \item After one circle the best individuals have taken over the entire population so the benefits of the Island model is gone, and it becomes a slow version of the Master/Slave model.
    \item Discuss why the Pool model was not able to find the same solution as the master/slave model.
\end{itemize}
